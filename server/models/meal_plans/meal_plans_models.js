...
exports.createMealPlan = async (data) => {
  ...
  try {
    const { provider_id, plan_name, price, description, is_active } = data;
    console.log(data);
    ...
    //   return {
    //     success: false,
    //     message: `A meal plan with the name '${plan_name}' already exists for the provider ID ${provider_id}.`,
    //   };
    // }
...
exports.editMealPlan = async ({ plan_id }) => {
  try {
    const { data, error } = await supabase
      .from("plans")
